    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http, SecurityProperties properties, ObjectProvider<LocalSessionFilter> localSessionFilter) throws Exception {
        ...
        if ("local".equalsIgnoreCase(properties.resolvedMode())) {
            ...
            http.authorizeHttpRequests(auth -> auth
                            ...
                            .requestMatchers(HttpMethod.GET, "/api/llm-settings", "/api/llm-settings/models").hasAnyAuthority(READ)
                            .requestMatchers(HttpMethod.PUT, "/api/llm-settings").hasAnyAuthority(ADMIN, ROLE_ADMIN)
                            .requestMatchers(HttpMethod.POST, "/api/chat").hasAnyAuthority(PLAN)
                            .requestMatchers(HttpMethod.GET, "/api/mcp-settings", "/api/mcp-settings/tools").hasAnyAuthority(ADMIN, ROLE_ADMIN)
                            .requestMatchers(HttpMethod.PUT, "/api/mcp-settings").hasAnyAuthority(ADMIN, ROLE_ADMIN)
                            .requestMatchers(HttpMethod.POST, "/api/mcp-settings/validate").hasAnyAuthority(ADMIN, ROLE_ADMIN)
                            ...
                            .requestMatchers(HttpMethod.POST, "/api/plans", "/api/plans/import", "/api/library/*/plans").hasAnyAuthority(PLAN)
                            .requestMatchers(HttpMethod.POST, "/api/plans/*/approve").hasAnyAuthority(APPROVE)
                            ...
                            .requestMatchers(HttpMethod.POST, "/api/runs/*/cancel").hasAnyAuthority(EXECUTE)
